...
from convs.linears import SimpleLinear, SplitCosineLinear, CosineLinear, MahalanobisSimilarityLearning
...
from convs.memo_resnet import  get_resnet34_imagenet as get_memo_resnet34 #for MEMO imagenet
...
def get_convnet(args, pretrained=False):
    ...
    elif name == 'memo_resnet34':
        _basenet, _adaptive_net = get_memo_resnet34()
        return _basenet, _adaptive_net
...
class BaseNet(nn.Module):
    ...
    def forward(self, x, labels=None, aug_mode=False):
        x = self.convnet(x,labels, aug_mode)
...
class MSLMovNet(BaseNet):
    ...
    def update_fc(self, nb_classes):
        ...
        if self.fc is not None:
            ...
            projections = copy.deepcopy(self.fc.projections.data)
            biases = copy.deepcopy(self.fc.biases.data)
            fc.projections.data[:nb_output] = projections
            fc.biases.data[:nb_output] = biases
    ...
    def generate_fc(self, in_dim, out_dim):
        fc = MahalanobisSimilarityLearning(in_dim, out_dim, r= self.r)
        return fc
...
class IncrementalNet(BaseNet):
    ...
    def update_morefc(self, num_old, num_total, task_size):
        fc = self.generate_fc(self.feature_dim, num_total + int((task_size-1)*task_size/2))
        if self.fc is not None:
            weight = copy.deepcopy(self.fc.weight.data)
            bias = copy.deepcopy(self.fc.bias.data)
    ...
    def forward(self, x, labels=None, aug_mode=False):
        x = self.convnet(x, labels= labels, aug_mode= aug_mode)
        out = self.fc(x["features"])
        out.update(x)
        if hasattr(self, "gradcam") and self.gradcam:
            out["gradcam_gradients"] = self._gradcam_gradients
    ...
    def update_fc(self, nb_classes, task_num):
        ...
        if self.fc is not None:
            if task_num == 1:
                ...
                fc.fc1.weight.data[:prev_out_features1] = self.fc.fc1.weight.data[:prev_out_features1]
                fc.fc1.weight.data[prev_out_features1:] = self.fc.fc2.weight.data[prev_out_features1:]
                fc.sigma.data = self.fc.sigma.data

        del self.fc
        self.fc = fc

    def update_morefc(self, num_old, num_total, task_size, task_num):
        fc = self.generate_fc(self.feature_dim, num_total + int((task_size-1)*task_size/2))
        if self.fc is not None:
            if task_num == 1:
                fc.fc1.weight.data = self.fc.weight.data
                ...
                fc.fc1.weight.data[:num_old] = self.fc.fc1.weight.data[:num_old]
                fc.fc1.weight.data[num_old:] = self.fc.fc2.weight.data[num_old:]
                fc.sigma.data = self.fc.sigma.data
        del self.fc
        self.fc = fc
...
class DERNet(nn.Module):
    ...
    def forward(self, x, labels=None, aug_mode=False):
        features = [convnet(x, labels, aug_mode)["features"] for convnet in self.convnets]
        features = torch.cat(features, 1)
    ...
    def update_morefc(self, num_old, num_total, task_size):
        ...
        if self.fc is not None:
            weight = copy.deepcopy(self.fc.weight.data)
            bias = copy.deepcopy(self.fc.bias.data)
            fc.weight.data[:num_old, : self.feature_dim - self.out_dim] = weight[:num_old]
            fc.bias.data[:num_old] = bias[:num_old]
        del self.fc
        self.fc = fc

        new_task_size = num_total - sum(self.task_sizes)
        self.task_sizes.append(new_task_size)

        self.aux_fc = self.generate_fc(self.out_dim, new_task_size+ int((task_size-1)*task_size/2) + 1)

    def update_ft(self, num_old, num_total, num_aux):
        if len(self.convnets) == 0:
            self.convnets.append(get_convnet(self.args))
        else:
            self.convnets.append(get_convnet(self.args))
        ...
        if self.out_dim is None:
            self.out_dim = self.convnets[-1].out_dim
        fc = self.generate_fc(self.feature_dim, num_total)
        if self.fc is not None:
            nb_output = self.fc.out_features
            weight = copy.deepcopy(self.fc.weight.data)
            bias = copy.deepcopy(self.fc.bias.data)
        ...
        new_task_size = num_total - num_old
        self.task_sizes.append(new_task_size)

        self.aux_fc = self.generate_fc(self.out_dim, new_task_size + num_aux + 1)
...
class FOSTERNet(nn.Module):
    ...
    def forward(self, x, labels= None, aug_mode= None):
        features_list = []
        fmaps = []

        for convnet in self.convnets:
            result = convnet(x, labels, aug_mode)
            features_list.append(result["features"])
            fmaps.append(result["fmaps"])

        features = torch.cat(features_list, 1)
        out = self.fc(features)
    ...
    def update_morefc(self, num_old, num_total, task_size):
        ...
        self.fc = fc
        self.task_sizes.append(task_size)
        self.fe_fc = self.generate_fc(self.out_dim, num_total + int((task_size-1)*task_size/2))
    ...
    def copy_fc(self, fc):
        ...
        self.fc.weight.data[:n, :m] = weight[:n, :m]
        self.fc.bias.data[:n] = bias[:n]
...
class AdaptiveNet(nn.Module):
    ...
    def forward(self, x, labels=None, aug_mode=False):
        base_feature_map = self.TaskAgnosticExtractor(x)
        features = [extractor(base_feature_map, labels, aug_mode) for extractor in self.AdaptiveExtractors]
        features = torch.cat(features, 1)
    ...
    def update_morefc(self, num_old, num_total, task_size):
        ...
        self.task_sizes.append(task_size)
        self.aux_fc=self.generate_fc(self.out_dim, num_total + int((task_size-1)*task_size/2))
