...
class iCaRL(BaseLearner):
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            self._network.train()
            losses, losses_clf, losses_kd = 0.0, 0.0, 0.0
            correct, total = 0, 0
            ...
            for i, (_, inputs, targets, domain_labels) in enumerate(train_loader):
                ...
                loss_kd = self._KD_loss(
                    ...
                    self._old_network(inputs)["logits"][:, :self._known_classes],
                    self.T,
                )
            ...
            if epoch % 20 == 0:
                test_acc = self._compute_accuracy(self._network, test_loader)
                info = "Task {}, Epoch {}/{} => Loss {:.3f}, Loss_clf {:.3f}, Loss_kd {:.3f}, Train_accy {:.2f}, Test_accy {}".format(
                    self._cur_task,
                    epoch + 1,
                    self.epochs,
                    losses / len(train_loader),
