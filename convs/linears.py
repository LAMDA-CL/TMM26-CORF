...
class MahalanobisSimilarityLearning(nn.Module):
    def __init__(self, in_features, out_features, r = 6, sigma=True):
        super(MahalanobisSimilarityLearning, self).__init__()
        
        self.in_features = in_features
        self.out_features = out_features
        self.r = r

        self.biases = nn.Parameter(torch.Tensor(self.out_features, self.in_features))
        
        self.projections = nn.Parameter(torch.Tensor(self.out_features, self.in_features, self.r))
    ...
    def reset_parameters(self):
        ...
        self.projections.data.uniform_(-stdv, stdv)
    ...
    def forward(self, input):
        ...
        Returns: Dict containing logits of shape (batch_size, out_features)
        """
        input_bias_subtracted = input.unsqueeze(1) - self.biases.unsqueeze(0)

        projected = torch.einsum('b c d, c d e -> b c e',input_bias_subtracted, self.projections)
        
        squared_sum = torch.sum(projected ** 2, dim=-1)
        
        max_vals, _ = torch.max(squared_sum, dim=1, keepdim=True)
        stable_exp = torch.exp(squared_sum - max_vals)
        
        sum_exp = torch.sum(stable_exp, dim=1, keepdim=True) + 1e-8
        normalized = stable_exp / sum_exp
